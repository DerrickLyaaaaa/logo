...
print('\t'.join([split,g(r'^AUROC:\s*([0-9.]+)$'),g(r'^FPR@TPR95:\s*([0-9.]+)$'),g(r'^w\*:\s*([0-9.]+)$')]))
PY
done

"$PYTHON_BIN" - "$OD/results.tsv" > "$OD/summary.tsv" <<'PY'
import csv,sys
rows=list(csv.DictReader(open(sys.argv[1],encoding='utf-8'), delimiter='\t'))
au=sum(float(r['AUROC']) for r in rows)/len(rows)
fp=sum(float(r['FPR95']) for r in rows)/len(rows)
print('macro_AUROC\tmacro_FPR95')
print(f'{au:.6f}\t{fp:.6f}')
PY

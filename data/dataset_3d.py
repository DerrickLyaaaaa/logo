...
class ScanObjectNN(data.Dataset):
    def __init__(self, config):
        ...
        split = config.subset
        self.subset = config.subset
        args_obj = getattr(config, 'args', None)
        custom_train_dat = getattr(args_obj, 'scanobject_train_dat', '') if args_obj is not None else ''
        custom_test_dat = getattr(args_obj, 'scanobject_test_dat', '') if args_obj is not None else ''
        custom_dat_path = custom_train_dat if split == 'train' else custom_test_dat
        ...
        if self.uniform:
            self.save_path = os.path.join(self.root,
                                          'scanobjectnn%d_%s_%dpts_fps.dat' % (self.num_category, split, self.npoints))
        else:
            self.save_path = os.path.join(self.root,
        ...
        if self.process_data:
            if custom_dat_path:
                self.save_path = custom_dat_path
                if not os.path.exists(self.save_path):
                    raise FileNotFoundError(f'Custom ScanObjectNN dat not found: {self.save_path}')
                print_log('Load custom processed data from %s...' % self.save_path, logger='ScanObjectNN')
                with open(self.save_path, 'rb') as f:
                    self.list_of_points, self.list_of_labels = pickle.load(f)
                ...
                if self.generate_from_raw_data:
                    ...
                    # no pre-processed dataset found and no raw data found, then load 8192 points dataset then do fps after.
                    self.save_path = os.path.join(self.root,
                                                  'scanobjectnn%d_%s_%dpts_fps.dat' % (
                                                  self.num_category, split, 8192))
    ...
    def _get_item(self, index):
        if self.process_data:
            ...
            label = np.array([int(label)], dtype=np.int32)
        else:
            fn = self.datapath[index]
            cls = self.classes[self.datapath[index][0]]
            label = np.array([cls]).astype(np.int32)
...
class ShapeNetCore(data.Dataset):
    def __init__(self, config):
        ...
        if self.uniform:
            self.save_path = os.path.join(self.root,
                                          'shapenetcore%d_%s_%dpts_fps.dat' % (self.num_category, split, self.npoints))
        else:
            self.save_path = os.path.join(self.root,
        ...
        if self.process_data:
            if not os.path.exists(self.save_path):
                ...
                if self.generate_from_raw_data:
                    ...
                    # no pre-processed dataset found and no raw data found, then load 8192 points dataset then do fps after.
                    self.save_path = os.path.join(self.root,
                                                  'shapenetcore%d_%s_%dpts_fps.dat' % (
                                                  self.num_category, split, 8192))
...
class Dataset_3D():
    def __init__(self, args, tokenizer, dataset_type, train_transform=None):
        ...
        with open('./data/dataset_catalog.json', 'r') as f:
            ...
            # Respect requested dataset_type when provided (train/val/test),
            # while keeping backward-compatibility via catalog "usage".
            if dataset_type in ['train', 'val', 'test']:
                split_key = 'train' if dataset_type == 'train' else 'test'
                self.dataset_usage = split_key
                self.dataset_split = entry[split_key]
            else:
                self.dataset_usage = entry['usage']
                self.dataset_split = entry[self.dataset_usage]

...
try:
    ...
    class _TorchKNN:
        """
        Fallback KNN when knn_cuda extension is unavailable.
        Keeps the same return interface used below.
        """
        ...
        def __call__(self, ref, query):
            # Expected by this file: ref/query are [B, 3, N] when transpose_mode=False.
            if not self.transpose_mode:
                ref_pts = ref.transpose(1, 2).contiguous()    # [B, N_ref, 3]
                query_pts = query.transpose(1, 2).contiguous()  # [B, N_query, 3]
            else:
                ref_pts = ref.contiguous()
                query_pts = query.contiguous()

            dist = torch.cdist(query_pts.float(), ref_pts.float(), p=2)
            d, idx = torch.topk(dist, k=self.k, dim=-1, largest=False, sorted=False)
            # Match knn_cuda layout used by call sites: [B, k, N_query]
            return d.permute(0, 2, 1).contiguous(), idx.permute(0, 2, 1).contiguous()

    knn = _TorchKNN(k=4, transpose_mode=False)
